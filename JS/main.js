...
(async function init(){
  ...
  const metrics=await getJSON('Data/metrics.json','metrics.json'); applyMetrics(metrics);
  const path=location.pathname.toLowerCase(); const onIndex=path.endsWith('/')||path.endsWith('/index.html'); const onProjects=path.endsWith('/projects.html'); const onPubs=path.endsWith('/publications.html');
  const projects=await getJSON('Data/projects.json','projects.json'); if(onIndex) renderProjects(projects,{containerSel:'#featured .card-grid',featuredOnly:true,limit=6}); if(onProjects) renderProjects(projects,{containerSel:'.card-grid',featuredOnly:false,limit:null});
  const pubs=await getJSON('Data/publications.json','publications.json'); if(onIndex) renderPublications(pubs,{containerSel:'#publications .pubs',selectedOnly:true,limit=5}); if(onPubs) renderPublications(pubs,{containerSel:'.pubs',selectedOnly:false,limit:null});
  const datasets=await getJSON('Data/datasets.json','datasets.json'); renderDatasets(datasets,{containerSel:'#datasets .list'});
})();